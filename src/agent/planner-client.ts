  ...
  public async planStep(request: PlannerRequest): Promise<PlannerResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Cloud Planner API error (${response.status}): ${errText || response.statusText}`);
      }

      const data = await response.json();
      return data as PlannerResponse;
    } catch (err: any) {
      if (err.message?.includes("Invalid schema_version")) {
        throw err;
      }
      throw new Error(`Cloud Reasoning Connection Failed [${this.endpointUrl}]: ${err.message}. Please verify that your cloud thinking model server is running and reachable.`);
    }
  }
}
